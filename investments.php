...
<body>
    ...
    <script>
        // Sistema de cores consistentes para badges (sincronizado com dashboard)
        const chartColors = [
            '#FFD700', // Dourado
            '#FF6B6B', // Vermelho
            '#4ECDC4', // Azul água
        ...
        function applyBadgeColors() {
            ...
            // Primeiro, identificar todos os tipos únicos
            badges.forEach(badge => {
                const tipoNome = badge.textContent.trim();
                if (!uniqueTypes.includes(tipoNome)) {
                    uniqueTypes.push(tipoNome);
                }
            });
            
            // Aplicar cores consistentes baseadas na ordem
            badges.forEach(badge => {
                const tipoNome = badge.textContent.trim();
                const colorIndex = uniqueTypes.indexOf(tipoNome);
                if (colorIndex >= 0) {
                    badge.style.backgroundColor = chartColors[colorIndex % chartColors.length];
                    badge.style.color = '#000'; // Texto preto para melhor contraste
                }
            });
        }

        // Aplicar cores quando a página carregar
