<?php
/**
 * PÁGINA PARA DELETAR META DE INVESTIMENTO
 */
...
if(!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    header('Location: metas.php?error=ID inválido');
    exit();
}
...
if($_POST && isset($_POST['confirm_delete'])) {
    ...
    if(empty($senha)) {
        ...
        try {
            $query = "DELETE FROM metas_investimento WHERE id = :id AND usuario_id = :usuario_id";
            $stmt = $conn->prepare($query);
            $stmt->bindParam(':id', $meta_id);
            $stmt->bindParam(':usuario_id', $user['id']);
            ...
            if($stmt->execute()) {
                ...
                $auditLog->logMeta($user['id'], 'delete', $meta_id, $metaData, null);
                
                header('Location: metas.php?success=Meta excluída com sucesso');
                exit();
            } else {
                $error = 'Erro ao excluir meta.';
            }
        } catch(Exception $e) {
            $error = 'Erro ao excluir meta: ' . $e->getMessage();
        }
    }
}
?>
...
<head>
    ...
    <title>Excluir Meta - Carteira de Investimentos</title>
    ...
    <div class="container">
        <div class="fade-in">
            <div class="page-header">
                <h1>🗑️ Excluir Meta</h1>
                <p style="color: #ccc;">Confirme a exclusão da meta</p>
            </div>
            ...
            <div class="card" style="max-width: 600px; margin: 0 auto;">
                ...
                <div class="card-body">
                    <div class="warning-box">
                        <p><strong>Você está prestes a excluir a seguinte meta:</strong></p>
                        
                        <div class="meta-details">
                            <div class="detail-row">
                                <span class="label">Título:</span>
                                <span class="value"><?= htmlspecialchars($metaData['titulo']) ?></span>
                            </div>
                            <div class="detail-row">
                                <span class="label">Tipo:</span>
                                <span class="value"><?= $metaData['tipo_meta'] === 'valor_total' ? 'Valor Total da Carteira' : 'Renda Passiva Mensal' ?></span>
                            </div>
                            <div class="detail-row">
                                <span class="label">Valor Meta:</span>
                                <span class="value">R$ <?= number_format($metaData['valor_meta'], 2, ',', '.') ?></span>
                            </div>
                            <div class="detail-row">
                                ...
                                <span class="value">
                                    ...
                                    switch($metaData['status']) {
                                        ...
                                        case 'pausado': echo '⏸️ Pausado'; break;
                                        default: echo $metaData['status'];
                                    }
                                    ?>
                                </span>
                            </div>
                            <?php if($metaData['data_limite']): ?>
                            <div class="detail-row">
                                <span class="label">Data Limite:</span>
                                <span class="value"><?= date('d/m/Y', strtotime($metaData['data_limite'])) ?></span>
                            </div>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="warning-text">
                            ...
                            <p style="color: #ccc;">
                                Todos os dados relacionados a esta meta, incluindo histórico e configurações, 
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p style="color: #FF4444; margin-bottom: 1rem;">
                    Para confirmar a exclusão desta meta, digite sua senha:
                </p>
