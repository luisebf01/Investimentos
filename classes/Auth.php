...
class Auth {
     ...
     */
    public function verifyCurrentUserPassword($senha) {
        // Verificar se o usuário está logado
        if(!$this->isLoggedIn()) {
            return false;
        }
        ...
        $query = "SELECT senha FROM usuarios WHERE id = :user_id AND ativo = 1";
        $stmt = $this->conn->prepare($query);
        ...
        if($stmt->rowCount() > 0) {
            ...
            // Verificar se a senha fornecida confere com a senha do banco
